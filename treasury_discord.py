    ...
    while True:
        try:
            async with session.get(iota_url, timeout=5) as resp:
                ...
                iota["watchlists"] = iotarsp["watchlist_portfolio_users"]
...
async def update_thread():
    ...
    while True: 
        if await update_status():
            ...
            config.set('watchlist', 'tanglehelper', TANGLEHELPER)
...
async def update_status():
    ...
    return upFarms or upTok or upLP or upTangle or upSwap  or upLend or upLum
...
async def main():
    ...
    asyncio.create_task(update_voting())
    global session
